...
**Test it works:**
Run `vulnforge scan --file testdata/pyproject.toml` and verify CRITICAL vulnerabilities have red backgrounds while LOW vulnerabilities use normal colors.
...
2. **Implement runStats()** that:
   - Lists all `.json` files in `~/.vulnforge/cache/`
...
**Test it works:**
Run vulnforge a few times with different packages, then `vulnforge stats` should show increasing entry count.
...
**Test it works:**
```bash
vulnforge scan --file testdata/pyproject.toml --json | jq '.vulnerabilities[].package'
```
...
**What to build:**
Extend vulnforge to handle requirements.in files (used by pip-tools for dependency compilation).

**Real world application:**
pip-tools workflow uses `requirements.in` for high-level dependencies and compiles to `requirements.txt`. vulnforge should update the .in file, then users run `pip-compile` to regenerate .txt.
...
- vulnforge should update .in, not .txt (which is auto-generated)

**Test it works:**
Create `test.in` with `requests>=2.28.0`, run `vulnforge update --file test.in`, verify version bumped and file preserved comments.
...
**What to build:**
Add `vulnforge sbom` command that outputs a CycloneDX or SPDX format SBOM.

**Why this is hard:**
SBOM formats are verbose and require data vulnforge doesn't currently collect (license info, package hashes, component relationships).
...
**Test it works:**
```bash
vulnforge sbom --file pyproject.toml --output sbom.xml
...
- How do you prevent infinite loops if vulnforge modifies the file it's watching?
...
- Store state in `~/.vulnforge/daemon_state.json`

**Phase 3: Notifications** (4-6 hours)
- Log new CVEs to `~/.vulnforge/daemon.log`
...
3. **Daemon lifecycle management**
   ...
   - Hint: `vulnforge daemon start --background` forks and exits. `vulnforge daemon stop` sends SIGTERM to PID file.
...
**Prerequisites:**
You should have a solid understanding of vulnforge's OSV client and interface design.
...
func (s *HTTPSource) ScanPackages(ctx context.Context, packages []Package) (map[string][]Vulnerability, error) {
    ...
    // Normalize response format to match vulnforge's Vulnerability struct
...
**The goal:**
Make vulnforge fast enough to scan monorepo dependency lists with thousands of packages.
...
unzip all.zip -d ~/.vulnforge/vulndb/
```
...
```bash
# Before optimization
time vulnforge scan --file huge-project.toml
# real    0m42.318s
...
**The goal:**
Make vulnforge compliant with OWASP Dependency-Check's requirements for SCA tools.
...
**The goal:**
Make vulnforge fetch vulnerability data from GitHub's GraphQL API in addition to OSV.dev.
...
3. **Convert to vulnforge's Vulnerability format**
...
**The goal:**
Run vulnforge as an AWS Lambda that scans repositories on webhook events.
...
1. **Containerize vulnforge**:
   ```dockerfile
   FROM golang:1.24 AS build
   WORKDIR /app
   COPY . .
   RUN go build -o vulnforge ./cmd/vulnforge
   
   FROM public.ecr.aws/lambda/provided:al2
   COPY --from=build /app/vulnforge /var/task/vulnforge
   ENTRYPOINT ["/var/task/vulnforge"]
   ```
...
1. **Fork the repo** at github.com/Kiliankm19/vulnforge
...
**vulnforge-watch** - Real-time dependency security monitor that opens GitHub issues when new CVEs appear. Combines file watching, GitHub API integration, and vulnerability scanning.

**vulnforge-diff** - Compare two branches/commits to see which dependencies changed and what security impact that has. Useful for pull request reviews.

**vulnforge-policy** - Policy as code engine. Define rules like "no dependencies with CRITICAL vulnerabilities" or "all deps must be updated within 30 days of release". vulnforge-policy enforces these in CI/CD.
...
- Step by step guide replicating vulnforge
- Explain each component
- Include exercises

**Comparison**: "vulnforge vs pip-audit vs Snyk"
- Feature matrix
...
2. **Read the existing code**
   - How does vulnforge handle similar problems?
