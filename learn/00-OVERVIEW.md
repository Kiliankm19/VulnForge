...
```bash
# Clone and navigate
cd PROJECTS/beginner/VulnForge
...
# Or run directly
go run ./cmd/vulnforge scan --file testdata/pyproject.toml
```

Expected output: vulnforge will show you that packages like `django>=3.2,<4.0` and `requests>=2.28.0` are outdated, then list any known vulnerabilities with their severity levels (CRITICAL, HIGH, MODERATE, LOW) and fixed versions.

To actually update a file:

```bash
# Check what would change (dry run)
go run ./cmd/vulnforge check --file testdata/pyproject.toml

# Update the file
go run ./cmd/vulnforge update --file testdata/pyproject.toml

# Update AND scan for vulnerabilities
go run ./cmd/vulnforge update --vulns --file testdata/pyproject.toml
```
...
│   ├── config/                  # vulnforge configuration loader
...
Solution: The PyPI Simple API has rate limits (roughly 10 requests/second). vulnforge defaults to 10 concurrent workers (`internal/pypi/client.go:17`). If you hit limits, reduce `DefaultMaxWorkers`. The cache helps avoid repeated requests.

**Cache shows stale data**
Solution: Clear the cache with `vulnforge cache clear` or manually delete `~/.vulnforge/cache/`. The default TTL is 1 hour (`internal/pypi/cache.go:11`). For development, you might want to lower this.
