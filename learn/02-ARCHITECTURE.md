...
This document breaks down how vulnforge is designed and why certain architectural decisions were made. We'll trace requests through the system and explain the tradeoffs.
...
- Responsibilities: Serialize JSON to `~/.vulnforge/cache/`, check TTL freshness, handle ETags
- Interfaces: `Get(key) (*CacheEntry, bool)`, `Set(key, entry) error`

**Config Loader** (`internal/config/`)
- Purpose: Read user preferences from `.vulnforge.toml` or `[tool.vulnforge]` in pyproject.toml
...
Step by step walkthrough of what happens when you run `vulnforge update --file pyproject.toml`:

```
1. CLI parses arguments → update.go:runUpdate() (internal/cli/update.go:200-291)
   Extracts file path, loads vulnforge config (ignore lists, min-severity)
...
**What it is:**
Instead of parsing TOML → modifying struct → re-serializing, vulnforge uses compiled regex patterns to locate and replace only the version specifier in the raw file bytes.
...
vulnforge uses a simple three-layer architecture:
...
**Fields explained:**
- `Name`: PEP 503 normalized package name. vulnforge uses `pypi.NormalizeName()` which converts to lowercase and treats `-`, `_`, `.` as equivalent.
...
- `Aliases`: Same vulnerability under different IDs. vulnforge uses this for deduplication.
...
1. **Installing vulnerable dependencies** - User has old package versions with known CVEs. vulnforge shows them which packages have fixes available and what the CVEs are.

2. **Accidentally upgrading to pre-releases** - User wants stable packages, but naive "latest version" logic might pick `3.0a1` over `2.5.0`. vulnforge filters pre-releases by default.

3. **Breaking changes from major bumps** - User has working code on `django==3.2`. Upgrading to `4.0` might break APIs. vulnforge's `--safe` flag skips major version bumps.

What we're NOT protecting against (out of scope):

- **Malicious code in dependencies** - vulnforge doesn't do static analysis or sandboxing. It trusts PyPI's package integrity.
- **Transitive dependency vulnerabilities** - vulnforge only scans packages explicitly in your dependency file. If `requests` depends on a vulnerable `urllib3`, vulnforge won't catch it unless you also list `urllib3`.
- **Supply chain attacks via package takeover** - If someone hijacks a legitimate package and pushes a backdoored version with no CVE yet, vulnforge won't detect it.

### Defense Layers

vulnforge implements defense in depth:
...
- **Cache with 1-hour TTL**: `internal/pypi/cache.go:11` sets `DefaultCacheTTL = 1 * time.Hour`. Running vulnforge twice within an hour has near-zero latency (cache hit).

- **Compiled regex patterns**: vulnforge compiles regex patterns once in `buildDepPattern()` rather than on every match. Go's `regexp.MustCompile()` at startup would be even better, but we need dynamic patterns (package name varies).

### Scalability

**Vertical scaling:**
vulnforge is CPU-bound during regex matching and memory-bound during concurrent HTTP requests. Adding more CPU cores doesn't help much (already using 10 goroutines). Adding RAM helps if you're scanning 1000+ dependencies (cache grows large).

**Horizontal scaling:**
Not applicable. vulnforge is a CLI tool that runs on one machine. If you wanted to scan 10,000 projects, you'd run 10,000 vulnforge processes in parallel (e.g., in Kubernetes jobs).
...
**Reasoning**: vulnforge needs to be fast (scan 50 packages in <1 second) and portable (run on any machine). Go compiles to a single binary with no runtime dependencies. Python would work but would be slower and require the right Python version installed.

### Decision 2: File-Based Cache vs In-Memory

**What we chose:**
File-based cache at `~/.vulnforge/cache/` with 1-hour TTL.
...
**Reasoning**: vulnforge's typical use case is "run it every hour in CI" or "run it when updating dependencies manually". File-based cache covers both. The cache size is small (50 packages × 5KB each = 250KB), so disk space isn't a concern.

## Deployment Architecture

vulnforge runs as a local CLI tool, not a server. There's no deployment topology. Users install the binary via:
...
vulnforge doesn't have plugins yet. If it did, the interface would look like:
...
Then vulnforge could support OSV.dev, Snyk, GitHub Advisory, and custom internal databases via the same interface. This is left as an exercise for readers (see 04-CHALLENGES.md).

## Limitations

Current architectural limitations:

1. **No transitive dependency scanning** - vulnforge only checks packages explicitly in your dependency file. If `requests` depends on a vulnerable `urllib3`, you won't know unless you also list `urllib3`. Fixing this requires a full dependency resolver (like pip's), which is complex.

2. **No offline mode** - vulnforge requires network access to PyPI and OSV.dev. Adding offline support would need bundled vulnerability database snapshots and local PyPI mirrors.

3. **Single project at a time** - You can't point vulnforge at 10 projects and get aggregated results. The CLI is designed for one `pyproject.toml` per invocation.
...
How we're different:
- pip-audit requires a Python installation and uses pip's resolver. vulnforge is a standalone Go binary.
- pip-audit scans installed packages (`pip list` output). vulnforge scans dependency files before installation.
- pip-audit doesn't update versions. vulnforge does both scanning and updating.

Why we made different choices: vulnforge is designed for CI/CD where you want to check *before* installing. pip-audit is better for auditing production environments.

### vs Dependabot

How we're different:
- Dependabot is a GitHub bot that opens PRs. vulnforge is a local CLI tool.
- Dependabot knows about all ecosystems (npm, PyPI, Maven, etc.). vulnforge only does PyPI.
- Dependabot has sophisticated version resolution with security vs compatibility tradeoffs. vulnforge has simple "latest stable" logic.

Why we made different choices: Dependabot is a production service with a team maintaining it. vulnforge is a learning project. The architecture reflects that (simple and hackable vs robust and comprehensive).

## Key Files Reference

Quick map of where to find things:

- `cmd/vulnforge/main.go` - Entry point, just calls cli.Execute()
...
3. Add a new command (like `vulnforge stats` to show cache hit rate) following the CLI patterns
