...
vulnforge queries PyPI's Simple API for version lists. A typical project has 20-50 dependencies. Without caching, you'd make 20-50 HTTP requests every time you run `vulnforge check`. That's slow (5-10 seconds) and rude (hammers PyPI).
...
Atomic write pattern: temp file + rename. If vulnforge crashes mid-write, the original cache entry is untouched. `os.Rename()` is atomic on POSIX systems.
...
Even if someone passes `../../../etc/passwd` as a package name, `filepath.Base()` returns just `passwd`, so the cache file goes in the cache directory as `passwd.json`. This prevents writing outside `~/.vulnforge/cache/`.
...
**Scenario:** User runs `vulnforge scan --file pyproject.toml`

### Request Comes In

Entry point: `cmd/vulnforge/main.go:7-9`
```go
func main() {
    cli.Execute()
}
...
vulnforge supports two config locations:
1. `.vulnforge.toml` in current directory
2. `[tool.vulnforge]` section in `pyproject.toml`
...
func Load(pyprojectPath string) Config {
    ...
    // Fall back to [tool.vulnforge] in pyproject.toml
...
The `loadFromPyproject()` function uses a wrapper struct to extract the `[tool.vulnforge]` section:
...
**Symptom:**
User has `Django>=3.2.0` in their pyproject.toml, but vulnforge says "package not found".
...
PEP 503 specifies package name normalization: lowercase, replace `[-_.]` with `-`. Both PyPI and vulnforge must use the same normalization or lookups fail.
...
**Symptom:**
User hits Ctrl+C, but vulnforge keeps running for 10 more seconds.
...
**Problem:** vulnforge makes fresh HTTP requests every time, ignoring cache.

**How to debug:**
1. Check `~/.vulnforge/cache/` for JSON files: `ls -lah ~/.vulnforge/cache/`
2. Look at timestamps: `cat ~/.vulnforge/cache/requests.json | jq '.cached_at'`
...
**Problem:** vulnforge updates a dependency but produces broken TOML.
...
Currently vulnforge preserves comments in pyproject.toml but not requirements.txt. Let's add it.
