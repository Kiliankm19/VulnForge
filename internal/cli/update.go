...
CLI commands and scan orchestration for vulnforge
...
import (
	...
	"github.com/Kiliankm19/vulnforge/internal/config"
	"github.com/Kiliankm19/vulnforge/internal/osv"
	"github.com/Kiliankm19/vulnforge/internal/pypi"
	"github.com/Kiliankm19/vulnforge/internal/pyproject"
	"github.com/Kiliankm19/vulnforge/internal/requirements"
	"github.com/Kiliankm19/vulnforge/internal/ui"
	"github.com/Kiliankm19/vulnforge/pkg/types"
...
func defaultCacheDir() string {
	...
	return filepath.Join(home, ".vulnforge", "cache")
}
...
func Execute() {
	root := &cobra.Command{
		...
		Long: `vulnforge scans your pyproject.toml, updates dependencies to their
...
func newInitCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "init",
		Short: "Create a new pyproject.toml with vulnforge configuration",
...
func runUpdate(
	...
	start := time.Now()
	cfg, cfgErr := config.Load(f.file)
	if cfgErr != nil {
		fmt.Fprintf(os.Stderr, "warning: could not load .vulnforge.toml: %v, using defaults\n", cfgErr)
	}
	...
	fetched, err := client.FetchAllVersions(ctx, names)
	if err != nil {
		return err
	}
...
func buildNewSpec(oldSpec, newVer string) string {
	switch {
	case strings.HasPrefix(oldSpec, "=="):
		return "== " + newVer
	case strings.HasPrefix(oldSpec, "~="):
		parts := strings.SplitN(newVer, ".", 3)
		if len(parts) >= 2 {
...
func resolveUpdates(
	...
	for _, dep := range deps {
		...
		newSpec := buildNewSpec(dep.Spec, latest.String())
		results = append(results, types.UpdateResult{
			Name:    dep.Name,
			OldVer:  current.String(),
			NewVer:  latest.String(),
