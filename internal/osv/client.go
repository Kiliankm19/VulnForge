...
var vulnIDRe = regexp.MustCompile(`^[A-Za-z0-9_\-\.]+$`)

const (
	batchEndpoint = "https://api.osv.dev/v1/querybatch"
	vulnEndpoint  = "https://api.osv.dev/v1/vulns/"
	userAgent     = "vulnforge/0.1.0 (https://github.com/Kiliankm19/vulnforge)"
	maxHydrate    = 15
...
func (c *Client) fetchVuln(
	...
	if !vulnIDRe.MatchString(id) {
		return nil, fmt.Errorf("invalid vuln ID: %q", id)
	}
	req, err := http.NewRequestWithContext(
